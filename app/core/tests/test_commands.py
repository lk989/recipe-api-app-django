...
@patch('core.management.commands.wait_for_db.Command.chech')
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_ready(self, patched_checkc):
        ...
        patched_checkc.assert_called_once_with(database=['default'])

    @patch('time.sleep')
    # arguments should be put from the inside out. like here: sleep before check
    def test_for_db_delay(self, patched_sleep, patched_checkc):
        ...
        patched_checkc.side_effect = [Psycopg2Error] * 2 + [OperationalError] * 3 + [True]

        call_command('wait_for_db')

        self.assertEqual(patched_checkc.call_count, 6)
        patched_checkc.assert_called_with(database=['default'])